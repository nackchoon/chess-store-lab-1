...
class DB
  class << self
    ...
    def create_users(session_id)
      ...
      user = User.new(username:'mheimann', password:'ilovegod', first_name:'Mark', last_name:'Heimann', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'lheimann', password:'captain', first_name:'Hack me?', last_name:'You are in trouble!', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
